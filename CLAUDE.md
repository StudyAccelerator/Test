...
- The diagnostic's follow-up emails live in MailerLite as four automations built 12 July 2026 (instant E0 on the "Revision Diagnostic" group + one linear automation per route group; route branching happens in `lib/mailerlite.ts` `routeGroupId()`, not in MailerLite condition steps). As of 14 July all four automations are switched on (Waleed confirmed; the earlier state had two routes off pending review). The HQ dashboard triage panel tracks the live on/off state, so check there rather than trusting this line. Full build detail: `content/email-sequences/revision-diagnostic/README.md`.
