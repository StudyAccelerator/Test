...
- The diagnostic's follow-up emails live in MailerLite as four automations built 12 July 2026 (instant E0 on the "Revision Diagnostic" group + one linear automation per route group; route branching happens in `lib/mailerlite.ts` `routeGroupId()`, not in MailerLite condition steps). All four are disabled until Waleed reviews them in the dashboard. Full build detail: `content/email-sequences/revision-diagnostic/README.md`.
