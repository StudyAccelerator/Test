...
/* Exported for api/lead-alert.ts (the webhook receiver), so the key lives in
   exactly one place. */
export const ML_API_KEY =
...
/* The Error Log tool was removed from the site on 28 July 2026 (Waleed's call:
   it returns as a PAID add-on, not a free tool). Its MailerLite group "Error
   Log" (193707705133696383) and el_* custom fields still exist in the account
   for when it comes back; the subscribe code lives in git history. */
