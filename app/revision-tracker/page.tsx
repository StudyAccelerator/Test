...
(function(){
  ...
  function planWeek(subjects){
    ...
    function sc(name){return subjPos[name]||'subj-pos-0';}
    ...
    ledger.forEach(function(t){
      ...
      for(let pass=0;pass<t.dwCount;pass++){
        ...
        for(let d=minD;d<=6&&placed<0;d++){if(fits(d,'deep',DEEP_DUR,t.subject)){placed=place(d,{type:'deep',subject:t.subject,topic:t.topic,dur:DEEP_DUR,colorClass:sc(t.subject)});t.dwDays.push(placed);}}
        // Relax minD gap constraint
        if(placed<0){for(let d=0;d<=6&&placed<0;d++){if(fits(d,'deep',DEEP_DUR,t.subject)){placed=place(d,{type:'deep',subject:t.subject,topic:t.topic,dur:DEEP_DUR,colorClass:sc(t.subject)});t.dwDays.push(placed);}}}
        // Last resort: ignore per-subject-per-day limit
        if(placed<0){for(let d=0;d<=6&&placed<0;d++){if(dMin[d]+DEEP_DUR<=DAY_CAP_STUDY&&dDeep[d]<MAX_DEEP_PER_DAY){placed=place(d,{type:'deep',subject:t.subject,topic:t.topic,dur:DEEP_DUR,colorClass:sc(t.subject)});t.dwDays.push(placed);}}}
        if(placed<0)break;
      }
    ...
    ledger.forEach(function(t){
      ...
      t.dwDays.forEach(function(dwD){
        [1,2,3].some(function(off){
          ...
          if(dMin[d]+RECALL_DUR<=DAY_CAP_STUDY){place(d,{type:'recall',subject:t.subject,topic:t.topic,dur:RECALL_DUR,colorClass:sc(t.subject)});return true;}
          return false;
        });
      });
    });
    ...
    ledger.forEach(function(t){
      ...
      t.dwDays.forEach(function(dwD){
        [3,4,5,6,2].some(function(off){
          ...
          if(dMin[d]+REVIEW_DUR<=DAY_CAP_STUDY){place(d,{type:'review',subject:t.subject,topic:t.topic,dur:REVIEW_DUR,colorClass:sc(t.subject)});return true;}
          return false;
        });
      });
    });
    ...
    for(let d=0;d<7;d++){
      ...
      while(dMin[d]<FILL_TO&&safety-->0){
        ...
        if(tAR<maxAR&&t.dwDay<d&&dMin[d]+RECALL_DUR<=FILL_TO){
          place(d,{type:'recall',subject:t.subject,topic:t.topic,dur:RECALL_DUR,colorClass:sc(t.subject)});
        }else if(tLR<maxLR&&dMin[d]+REVIEW_DUR<=FILL_TO){
          place(d,{type:'review',subject:t.subject,topic:t.topic,dur:REVIEW_DUR,colorClass:sc(t.subject)});
        }
  ...
  function buildDayEvents(dayIndex,state,plannedSessions){
    ...
    while(cursor<CUTOFF&&safety-->0){
      ...
      const isAfternoon=cursor>=lunchMin&&cursor<dinnerMin;
      const brkDur=ev.type==='deep'?(isAfternoon?45:30):ev.type==='recall'?(isAfternoon?30:20):(isAfternoon?25:15);
  ...
  document.getElementById('print-btn').addEventListener('click',function(){
    ...
    wrap.style.cssText='width:1380px;background:#fff;padding:10px 14px;box-sizing:border-box;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif';

    const title=document.createElement('div');
    title.style.cssText='font-size:14px;font-weight:700;color:#2E2557;margin-bottom:6px;text-align:center;letter-spacing:0.02em';
    ...
    legClone.style.cssText='margin-top:6px;background:#fff;border-radius:4px;padding:5px 8px;box-shadow:none';
    ...
    setTimeout(function(){
      // Physically scale positioned elements in the grid so the whole plan fits on one A4 landscape page.
      // At windowWidth=1380px, jsPDF (3mm margins) x-scales content to 291mm.
      // Max wrap height = 204mm × (1380px / 291mm) = 967px.
      const MAX_H=967;
      const gridClone=wwClone.querySelector('#week-grid');
      const totalH=wrap.scrollHeight;
      if(totalH>MAX_H&&gridClone){
        const nonGridH=totalH-gridClone.scrollHeight;
        const sf=Math.max(0.35,(MAX_H-nonGridH)/gridClone.scrollHeight);
        gridClone.querySelectorAll('.tt-day-col,.tt-axis').forEach(function(col){
          if(col.style.height)col.style.height=Math.round(parseFloat(col.style.height)*sf)+'px';
          col.querySelectorAll('[style]').forEach(function(el){
            if(el.style.top)el.style.top=Math.round(parseFloat(el.style.top)*sf)+'px';
            if(el.style.height&&parseFloat(el.style.height)>0)
              el.style.height=Math.max(10,Math.round(parseFloat(el.style.height)*sf))+'px';
          });
        });
      }
      html2pdf().set({
        ...
        html2canvas:{scale:2,useCORS:true,logging:false,scrollX:0,scrollY:0,windowWidth:1380,allowTaint:true},
