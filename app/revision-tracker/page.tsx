...
(function(){
  ...
  function addSubjectRow(name,topics,priority,confidence){
    name=name||'';topics=topics||'';priority=priority||'medium';confidence=confidence||3;
    subjectCounter++;
    const id='subject-'+subjectCounter;
    const el=document.createElement('div');
    el.className='item-row subject-row';el.id=id;
    el.innerHTML='<button type="button" class="remove-btn" aria-label="Remove" title="Remove">×</button>'+
      '<label>Subject name</label>'+
      '<input type="text" class="subject-name" placeholder="e.g. Mathematics" value="'+escapeHTML(name)+'" maxlength="40">'+
      '<label>Topics to prioritise this week</label>'+
      '<input type="text" class="subject-topics" placeholder="e.g. Organic mechanisms, Electrochemistry" value="'+escapeHTML(topics)+'" maxlength="120">'+
      '<p class="help-text">Comma-separated. These appear on the timetable slots.</p>'+
      '<label>Priority level</label>'+
      '<select class="subject-priority">'+
        '<option value="high"'+(priority==='high'?' selected':'')+'>High — I need this grade</option>'+
        '<option value="medium"'+(priority==='medium'?' selected':'')+'>Medium — Important but manageable</option>'+
        '<option value="low"'+(priority==='low'?' selected':'')+'>Low — Fairly confident</option>'+
      '</select>'+
      '<div class="slider-wrap">'+
        '<div class="slider-head"><label>Confidence level</label><span class="conf-badge">'+confidence+'/5</span></div>'+
        '<input type="range" class="subject-confidence" min="1" max="5" value="'+confidence+'">'+
        '<div class="slider-labels"><span>1 · very low</span><span>3 · okay</span><span>5 · very high</span></div>'+
      '</div>';
    document.getElementById('subjects-list').appendChild(el);
    const range=el.querySelector('.subject-confidence');
    const badge=el.querySelector('.conf-badge');
    range.addEventListener('input',function(){badge.textContent=range.value+'/5'});
    el.querySelector('.remove-btn').addEventListener('click',function(){el.remove()});
  }
  let commitCounter=0;
  function addCommitmentRow(day,start,end,label){
    day=day||'Monday';start=start||'17:00';end=end||'18:30';label=label||'';
    commitCounter++;
    const id='commit-'+commitCounter;
    const el=document.createElement('div');
    el.className='item-row commit-row';el.id=id;
    const dayOpts=DAYS.map(function(d){return'<option value="'+d+'"'+(d===day?' selected':'')+'>'+d+'</option>'}).join('');
    el.innerHTML='<button type="button" class="remove-btn" aria-label="Remove" title="Remove">×</button>'+
      '<div class="commit-grid">'+
        '<div><label>Day</label><select class="commit-day">'+dayOpts+'</select></div>'+
        '<div><label>Start</label><input type="time" class="commit-start" value="'+escapeHTML(start)+'"></div>'+
        '<div><label>End</label><input type="time" class="commit-end" value="'+escapeHTML(end)+'"></div>'+
        '<div><label>What is it?</label><input type="text" class="commit-label" placeholder="e.g. Football" value="'+escapeHTML(label)+'" maxlength="40"></div>'+
      '</div>';
  ...
  function collectState(){
    ...
    document.querySelectorAll('.subject-row').forEach(function(row){
      const sname=row.querySelector('.subject-name').value.trim();
      if(!sname)return;
      subjects.push({name:sname,topics:row.querySelector('.subject-topics').value.trim(),priority:row.querySelector('.subject-priority').value,confidence:parseInt(row.querySelector('.subject-confidence').value,10)||3});
    });
    ...
    document.querySelectorAll('.commit-row').forEach(function(row){
      const lbl=row.querySelector('.commit-label').value.trim();
      const start=row.querySelector('.commit-start').value;
      const end=row.querySelector('.commit-end').value;
      if(!lbl||!start||!end)return;
    ...
    return{name:name,wakeTime:wakeTime,subjects:subjects,commitments:commitments};
  }
  function buildDaySlots(dayIndex,state,sortedSubjects){
    const DEEP=90,BRK=15,LUNCH=60;
    const CUTOFF=toMinutes('21:00');
    const EVENING=toMinutes('18:00');
    const LUNCH_START=toMinutes('12:00');
    const slots=[];
    const dayName=DAYS[dayIndex];
    const todayCommits=state.commitments.filter(function(c){return c.day===dayName}).sort(function(a,b){return a.startMin-b.startMin});
    let current=toMinutes(state.wakeTime)+60;
    if(isNaN(current))current=toMinutes('08:00');
    let lunchTaken=false,deepCount=0,recallCount=0,eveningDone=false;
    const added=new Set();
    let guard=0;
    function nextSubj(offset){return sortedSubjects[(dayIndex+offset)%sortedSubjects.length]}
    function subjSub(s){return s.topics?s.topics:'Deep focus — no distractions'}
    function recallSub(s){return s.topics?s.topics:'Past papers · blurting · Anki'}
    while(current<CUTOFF&&guard<60){
      guard++;
      const active=todayCommits.find(function(c){return c.startMin<=current&&c.endMin>current&&!added.has(c)});
      if(active){slots.push({type:'fixed',time:fmtTimeRange(active.startMin,active.endMin),label:active.label,sub:''});added.add(active);current=active.endMin;continue}
      const pastActive=todayCommits.find(function(c){return c.startMin<=current&&c.endMin>current&&added.has(c)});
      if(pastActive){current=pastActive.endMin;continue}
      const upcoming=todayCommits.find(function(c){return c.startMin>current&&c.startMin<current+DEEP&&!added.has(c)});
      if(upcoming){if(upcoming.startMin-current>=20)slots.push({type:'free',time:fmtTimeRange(current,upcoming.startMin),label:'Free / Buffer',sub:''});current=upcoming.startMin;continue}
      if(!lunchTaken&&current>=LUNCH_START&&current<EVENING){
        const lunchEnd=current+LUNCH;
        const conflict=todayCommits.find(function(c){return c.startMin<lunchEnd&&c.endMin>current});
        if(!conflict){slots.push({type:'lunch',time:fmtTimeRange(current,lunchEnd),label:'Lunch Break',sub:'Step away from the desk'});current=lunchEnd;lunchTaken=true;continue}
        else{lunchTaken=true}
      }
      if(current+DEEP>CUTOFF)break;
      if(current>=EVENING){
        if(eveningDone)break;
        const subj=nextSubj(0);
        slots.push({type:'review',time:fmtTimeRange(current,current+DEEP),label:'Light Review: '+subj.name,sub:'Consolidate notes · mind maps'});
        current+=DEEP;eveningDone=true;continue;
      }
      if(deepCount<2){
        const subj=nextSubj(deepCount);
        slots.push({type:'deep',time:fmtTimeRange(current,current+DEEP),label:subj.name,sub:subjSub(subj)});
        current+=DEEP;deepCount++;
      } else {
        const subj=nextSubj(deepCount+recallCount);
        slots.push({type:'recall',time:fmtTimeRange(current,current+DEEP),label:'Active Recall: '+subj.name,sub:recallSub(subj)});
        current+=DEEP;recallCount++;
      }
      if(current+BRK<CUTOFF&&current+DEEP+BRK<=CUTOFF){
        const brkEnd=current+BRK;
        const conflict=todayCommits.find(function(c){return c.startMin<brkEnd&&c.endMin>current&&!added.has(c)});
        if(!conflict){slots.push({type:'break',time:fmtTimeRange(current,brkEnd),label:'Short Break',sub:'Hydrate · move · reset'});current=brkEnd}
      }
    }
    todayCommits.forEach(function(c){if(!added.has(c))slots.push({type:'fixed',time:fmtTimeRange(c.startMin,c.endMin),label:c.label,sub:''})});
    if(slots.length===0)slots.push({type:'none',time:'',label:'No study slots',sub:'Check your wake time or commitments'});
    return slots;
  }
  function generatePlan(state){
    const sorted=[...state.subjects].sort(function(a,b){return(PRIORITY_WEIGHT[b.priority]*(6-b.confidence))-(PRIORITY_WEIGHT[a.priority]*(6-a.confidence))});
    const week=[];
    for(let i=0;i<7;i++)week.push({dayName:DAYS[i],slots:buildDaySlots(i,state,sorted)});
    return week;
  }
  function renderTimetable(week,name){
    ...
    week.forEach(function(day){
      ...
      day.slots.forEach(function(slot){
        ...
        let html='';
        if(slot.time)html+='<span class="slot-time">'+escapeHTML(slot.time)+'</span>';
        html+='<span class="slot-label">'+escapeHTML(slot.label)+'</span>';
  ...
  document.getElementById('add-subject-btn').addEventListener('click',function(){addSubjectRow()});
  document.getElementById('add-commit-btn').addEventListener('click',function(){addCommitmentRow()});
  document.getElementById('generate-btn').addEventListener('click',function(){
    const state=collectState();
    if(!state)return;
    renderTimetable(generatePlan(state),state.name);
    showTimetable();
  });
...
export default function RevisionTrackerPage() {
  useEffect(() => {
    ...
    return () => {
      ...
      if (sl) sl.innerHTML = ''
      const cl = document.getElementById('commitments-list')
      if (cl) cl.innerHTML = ''
    }
  }, [])
      ...
      <div className="container">
        <section id="questionnaire-section">
          ...
          <div className="card">
            ...
            <div className="info-line">Study cut-off is fixed at <strong>9:00 PM</strong> — non-negotiable. Sleep is part of the system.</div>
          </div>
        ...
        <section id="timetable-section">
          <div className="tt-header">
            ...
            <p>90-minute deep-work blocks · 15-minute breaks · Study cut-off 9:00 PM</p>
          </div>
