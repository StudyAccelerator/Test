...
- Diagnostic follow-up emails: a 16 email sequence (instant report email plus five emails over eleven days in three variants, branched on the student's recommended route) is fully built in MailerLite as four automations with all copy loaded, switched off until Waleed reviews each email in the dashboard and the sending domain is authenticated. Source copy lives in the repo at content/email-sequences/revision-diagnostic/.
