...
| Lead CRM | Auto, every 20 min | The **Lead CRM** panel (Today section) holds every lead the site has ever captured, one card each, pulled from the MailerLite capture groups (both diagnostic groups, Callback requested, Revision Tracker, Parents' Guide, Sunday Session) into `data/leads-crm.json`. The MailerLite half of each card (who they are, what the diagnostic found, the number and call preference, how many emails they have been sent, opened and clicked) refreshes on every sync; the checklist (personal email sent, called with outcome, text follow-up, Loom sent, call booked, call held, enrolled), status, next action and notes are Waleed's, typed in the panel, and a sync never overwrites them. Filters: Needs action, Ready to call, Callback requests, Open, Closed, Everyone. Uncalled callable leads are promoted into triage and counted on the phone widget as `callsDue`. Edits go through `POST /api/leads-crm/update` (one lead at a time) so a sync landing mid-edit never loses anything; `POST /api/leads-crm/sync` forces a refresh. Built 21 August 2026. |
