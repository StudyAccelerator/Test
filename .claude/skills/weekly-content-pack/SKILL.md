...
- **Waleed's OWN group (once "A-Level Parents UK" is live; launch kit `content/facebook-groups/2026-08-06-group-launch-kit.md`):** every pack includes a "group posts" section with four posts a week on the group's rhythm (Monday conversation question, Wednesday method post, Friday AMA thread, plus one story or proof post), written to the same typing-test rules as page posts. These are separate drafts from the page posts, never copies, and the group's transparency promise holds: programme mentions only in clearly marked posts, never in comments. Until the group exists this section is skipped without comment.
