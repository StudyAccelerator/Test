...
3. **Place it in the calendar.** The opening timestamps the email in the A-level year (first tests, predicted grades, mocks, results day) and says why this matters now. Check dates against `audience-and-offers.md` and the site.
4. **Write to the framework** (`references/framework.md`). Warm opener, why it matters now, the problem, the medical comparison if it helps, the teach in numbered bold steps, "tonight, try this", the bridge to the offer, the offer as a reward, the proof line, the named link, sign-off, PS with the link again.
5. **Write the subject lines and preview text** (`references/subject-lines.md`). Two subjects (A and B) and a preheader that adds a second hook rather than repeating the subject.
6. **Run the three checks** (below). Fix everything they name before anyone reads it.
