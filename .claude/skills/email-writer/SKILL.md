...
5. **Write to the framework** (`references/framework.md`). Salutation then straight in (no "I hope you're well" except on an automation's first email), why it matters now, the problem, the medical comparison if it helps, the teach in numbered bold steps, "tonight, try this", the bridge to the offer, the offer as a reward, the proof line, the named link, sign-off, PS with the link again.
