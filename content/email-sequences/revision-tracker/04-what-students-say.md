# T4 · What changes first

Send: Day 5
Subject A: what changes first (it isn't the grades)
Subject B: the number that matters most
Preheader: What students say when structured revision clicks, and a free half hour if you want eyes on your week.
Goal: proof email plus the first call offer. Real quotes, the two-grade average, one CTA: the call.
Links: 1 (call scheduler)
Revised: 6 September 2026 (Waleed's style pass: evidence disclaimers out, reward framing and the ask in return on the call)

---

Hi {$name},

You're five days into running a real revision system. So today, less method from me, and more from students who've been where you are.

Jay, Year 12: "The lessons were interactive with well explained concepts, easy to follow and very informative. Even the best students had something to improve on."

Delicia, Year 13: "The topics were really hard to approach while researching alone, so seeing it broken down into manageable points was useful."

Biju, who came to us during a gap year: "All the information provided was clear and understandable. It was really helpful and improved my confidence!"

Notice the word that keeps coming up: confidence, not grades. That's because confidence is what changes first. The grades follow, and on average our students jump two grades. If you've run even three honest sessions this week, you might have felt the start of it already.

So here's a reward for getting this far into the week, because most students don't. I keep a few free 30 minute calls each week for students keen to improve their grades. We look at your tracker week and how it's actually going, I tell you what I'd change first and why, and you leave with a strategy plan for the next two weeks. No cost, and all I ask in return is that you keep me updated on how it goes :)

[BUTTON: Book a free 30 minute call -> https://scheduler.zoom.us/dr-waleed-ahmad/a-level]

Keep going!

Waleed

PS: If none of the slots work around school, just reply with a couple of evenings that do and we'll sort a time. I'm a full time doctor, so bear with me if I'm on nights that week haha.
