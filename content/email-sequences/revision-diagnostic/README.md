...
Later additions: the gate captures an optional **phone** (built-in field, call follow-up only, per the on-page promise) and, since 15 July, the diagnostic FORKS at the start into student and parent paths. **Parents never enter this sequence any more**: they join separate parent groups and receive the dedicated buyer sequence in `../revision-diagnostic-parents/`. `diag_taker` records which path was taken; `diag_support` (tutoring status) and `diag_notes` (free text) arrive from both paths and are useful context on calls.
