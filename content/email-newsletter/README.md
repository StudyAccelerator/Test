...
- All seven issues are loaded as **complete draft campaigns** on the reshuffled calendar, body included, each ending in Waleed's **photo signature** (his real headshot on the MailerLite CDN, `account_image/2113061/3SoYgyuLfUdmX53Lnw82S2YV4c6PnZsQch9dP7T5.jpg`). The signature block is now his **verbatim builder-campaign signature** (80px round photo, auto height so it never distorts, "Kind regards / Dr Waleed Ahmad / Founder of A-Level Accelerators / email"): an earlier 64x64 restyle squashed the photo and Waleed flagged it on review. SS1 also opens with a short "I'm Dr Waleed" introduction (his request: many list members joined via workshops months ago and may not know him). Current IDs: SS1 launch `194711467593827416` (fixed signature + intro), SS2 playbook `194701575222985804`, SSX special `193067462770558621`, SS3 doctor `194701390896956464`, SS4 prediction `194701434790347781`, SS5 backwards `194701630741939763`, SS6 ugly notes `194701670822708558` (SS2 to SS6 still carry the old distorted signature; sweep them via delete+recreate from the rebuilt `emails4` fragments, SS2 first as it sends 9 Aug). (IDs change whenever copy changes because campaigns must be deleted and recreated, see the gotcha below.) Each has subject, sender ("Dr Waleed | A-Level Accelerators", waleed@alevelaccelerators.com), preheader (hidden div in the HTML) and the ten-group send list (~429 recipients). None is scheduled: Waleed reviews and schedules each, which is the approval gate.
