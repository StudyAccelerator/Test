...
**From 13 September 2026 the Session is written with the `email-writer` skill (`.claude/skills/email-writer/`) and built by the engine, which supersedes the running order, the build-email-html.py flow and the MailerLite template steps below.** Each issue is two files: the student version (`YYYY-MM-DD-slug.md`) and the parent twin (`YYYY-MM-DD-slug-parents.md`), both in engine markdown; `manifest.json` in this folder maps them to two campaigns (students + mixed list, parent groups). `python3 scripts/email-engine/engine.py campaigns content/email-newsletter --live` creates the drafts (`--update` rewrites them); scheduling happens only on Waleed's go. Review goes through a MailerLite test send (the engine's `harness` command), never the Gmail connector. The first issue in this form, "marks per minute" (13 September), is the exemplar. The sections below stay as the history and the topic bank.
