...
### Loaded state (14 July 2026)

- The MailerLite connector is authorised on Waleed's Claude account; sessions now write to MailerLite through it.
- Group **Sunday Session** exists: `192801700892903405`.
- All seven launch-run issues are loaded as **draft campaigns** (SS1 `192992825977079426`, SS2 `192992850718230025`, SS3 `192992853289338825`, SS4 `192992870772245525`, SSX `192992891278198667`, SS5 `192992906187900363`, SS6 `192992926699095487`), each with subject, sender ("Dr Waleed | A-Level Accelerators", waleed@alevelaccelerators.com) and the full send list (423 recipients) preset. None is scheduled; Waleed schedules each after review.
- **Plan gate:** the account's MailerLite plan rejects HTML content via API ("content submission is only available on Premium plan"), so the drafts carry everything except the body. Upgrading the MailerLite plan lets a session inject all seven bodies in one pass; otherwise paste each issue's HTML (generated by `build-email-html.py` in this folder, run from the repo root) into the campaign's custom HTML editor at review time.
- `build-email-html.py` converts every issue file to final email HTML (masthead, template styles, preheader as hidden div, footer with unsubscribe). Output dir is its first argument.
