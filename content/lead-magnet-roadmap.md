...
- **Gated 22 July 2026 on Waleed's call** (his verdict on review: too valuable to hand out cold, and a candidate to bundle into the paid programmes). A one-time signup opens the tool and captures the richest profile of any free tool: name, email, year group, every subject with exam board and current to target grade, an open "anything I should know" box, and optional phone. It lands in MailerLite group "Error Log" (193707705133696383) as el_subjects / el_notes / el_date plus the shared name, year_group, subjects and phone fields; the device then never re-gates. The mistakes a student logs still never leave their browser, and the page says so. Rules (a) and (b) both now satisfied: it captures the daily-habit segment the one-shot diagnostic misses AND deepens the record per person. Retest nudges ship as a generated calendar file (all-day event per due date with an alarm), the serverless answer to notifications; true push would need a small backend, noted below.
- **Still to build for the gate:** a welcome automation on the "Error Log" group (a short "how to run your log" email would make the promise in the signup microcopy concrete), and the paid-programme bundle framing if Waleed goes that way.
